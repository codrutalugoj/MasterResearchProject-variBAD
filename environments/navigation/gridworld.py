    ...
    def visualise_behaviour(env,
        ...
        for episode_idx in range(args.max_rollouts_per_task):
            ...
            if encoder is not None:
                ...
                if episode_idx == 0:
                    # reset to prior
                    curr_latent_sample, curr_latent_mean, curr_latent_logvar, hidden_state, curr_precision = encoder.prior(1)
                    curr_latent_sample = curr_latent_sample[0].to(device)
                    curr_latent_mean = curr_latent_mean[0].to(device)
                    curr_latent_logvar = curr_latent_logvar[0].to(device)
                    curr_precision = curr_precision[0].to(device)
            ...
            for step_idx in range(1, env._max_episode_steps + 1):
                ...
                if encoder is not None:
                    ...
                    curr_latent_sample, curr_latent_mean, curr_latent_logvar, hidden_state, curr_precision = encoder(
                        action.float().to(device),
                        state,
                        rew_raw.reshape((1, 1)).float().to(device),
                        hidden_state,
                        old_means=curr_latent_mean,
                        old_precision=curr_precision,
                        return_prior=False)
