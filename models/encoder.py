...
class RNNEncoder(nn.Module):
    ...
    def prior(self, batch_size, sample=True):
        # TODO: does self.latent_dim make sense here? This is the shape that outl2 layer in meta_mu outputs
        hidden_state = torch.zeros((1, batch_size, self.hidden_size), requires_grad=True).to(device)
        hidden_mean = torch.zeros((1, batch_size, self.hidden_size), requires_grad=True).to(device)
        hidden_precision = self.learnable_vars.expand((-1, batch_size, -1))
        ...
        return latent_sample, latent_mean, latent_logvar, hidden_state, hidden_mean, hidden_precision
    ...
    def forward(self, actions, states, rewards, hidden_state, old_precision, old_means, return_prior, sample=True, detach_every=None):
        ...
        return latent_sample, latent_mean, latent_logvar, output, hidden_means, hidden_precisions
